...
const ContactApiEndpoint =
  process.env.ContactApiEndpoint ||
  "https://gcj1shtql7.execute-api.eu-west-1.amazonaws.com/v1/contact";
const FileSuffix = process.env.FileSuffix
  ? "." + process.env.FileSuffix.replace(/^\./, "")
  : ".html";
...
const buildVars = {
  ...
  linkTo: function (path) {
    if (!path) return AppRootURL;
      ...
      return AppRootURL + "/" + path.replace(/^\//, "");
    else
      return (
        AppRootURL.replace(/\/$/, "") +
        "/" +
...
async function getPosts(postTypes) {
  ...
  for (var type of postTypes) {
    if (type === "posts" || type === "pages") {
      ...
      await Promise.all(
        fileList.map(async function (filename) {
          ...
          if (__parsed.body)
            throw new Error(
              "post " + filename + ' is using reserved key "body"'
            );
...
function buildPublicPath(page) {
  ...
  if (page.path && page.path.length > 0) {
    ...
    if (_path.match(/\/$/)) _publicPath = _path + "index";
    // replace roots with index
    else _publicPath = _path;
  }
  if (FileSuffix.length > 0)
    _publicPath = _publicPath + "." + FileSuffix.replace(/^\./, "");

  if (page.basePath && page.basePath.length > 0) {
    // ensure path starts with leading / and ends without /
    let _basePath =
      "/" + page.basePath.replace(/^[\/]+/, "").replace(/[\/]+$/, "");
  ...
  if (_publicPath.match(/^\/assets\//)) {
    throw new Error(
      'Path can\'t begin with reserved path "/assets". Occured in file ' +
        page._absolutePath
    );
  }
...
function generateHTML(page) {
  ...
  if (!fse.existsSync(template_file))
    throw new Error(
      `Template '${template}' specified in ${page._absolutePath} does not exist`
    );

  return new Promise(function (resolve, reject) {
    ejs.renderFile(template_file, Object.assign({}, page, buildVars), function (
      err,
      body
    ) {
      if (err) reject(err);
...
function buildSearchIndex(pages) {
  ...
  function cleanUpBody(string) {
    return string
      .replace(/<[^>]+>/g, " ")
      .replace(
        /[^0-9a-zA-ZÆÐƎƏƐƔĲŊŒẞÞǷȜæðǝəɛɣĳŋœĸſßþƿȝĄƁÇĐƊĘĦĮƘŁØƠŞȘŢȚŦŲƯY̨Ƴąɓçđɗęħįƙłøơşșţțŧųưy̨ƴÁÀÂÄǍĂĀÃÅǺĄÆǼǢƁĆĊĈČÇĎḌĐƊÐÉÈĖÊËĚĔĒĘẸƎƏƐĠĜǦĞĢƔáàâäǎăāãåǻąæǽǣɓćċĉčçďḍđɗðéèėêëěĕēęẹǝəɛġĝǧğģɣĤḤĦIÍÌİÎÏǏĬĪĨĮỊĲĴĶƘĹĻŁĽĿʼNŃN̈ŇÑŅŊÓÒÔÖǑŎŌÕŐỌØǾƠŒĥḥħıíìiîïǐĭīĩįịĳĵķƙĸĺļłľŀŉńn̈ňñņŋóòôöǒŏōõőọøǿơœŔŘŖŚŜŠŞȘṢẞŤŢṬŦÞÚÙÛÜǓŬŪŨŰŮŲỤƯẂẀŴẄǷÝỲŶŸȲỸƳŹŻŽẒŕřŗſśŝšşșṣßťţṭŧþúùûüǔŭūũűůųụưẃẁŵẅƿýỳŷÿȳỹƴźżžẓ\s-,.\']/g,
        " "
      )
      .replace(/(\s|\r\n|\n)+/g, " ");
  }
    ...
    let index = lunr(function (builder) {
      ...
      for (var i = 0; i < pages.length; i++) {
        ...
        if (uri) {
          ...
          console.log(
            "UNABLE TO ADD FILE TO SEARCH INDEX, NO IDENTIFIER (uri or path)"
          );
        }
      }
    });
...
async function build() {
  ...
  for (var i = 0; i < pages.length; i++) {
    if (!validatePageData(pages[i])) {
      console.log("page " + pages[i]._absolutePath + " not valid, skipping");
      continue;
    }
    ...
    if (
      typeof pages[i].hideFromSearch === "undefined" ||
      !pages[i].hideFromSearch
    ) {
      searchables.push({
  ...
  await Promise.all(
    assets.map(async function (file) {
      await fse.copy(
        `${__dirname}/_assets/${file}`,
        `${PUBLIC_DIR}/assets/${file}`
      );
      if (file.match(/\.pdf$/)) {
        try {
          let pdfContent = await extractPDFContent(
            `${__dirname}/_assets/${file}`
          );
          let pdfRows = pdfContent
            .split("\n")
            .filter((r) => {
  ...
  await fse.writeFile(
    `${PUBLIC_DIR}/assets/${SEARCH_INDEX_FILE}`,
